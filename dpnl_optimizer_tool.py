...
def find_s_optimum_from_counts(dt: int, edu_sp: int, dp: int, brdu_sp: int, print_only=False):
    ...
    dt - the waiting time between labels used in a pilot dual nucleoside labeling assay. Must be to the nearest hour.
...
def find_s_optimum_from_phases(tg1: float, ts: float, tc: float, print_only=False):
    '''
    Function takes the following:
    tg1 - estimated duration of G1 phase
    ts - estimated duration of S phase
    tc - estimated duration of whole cell cycle
    ...
    relative_times = np.array([tg1, ts, tc-tg1-ts])/tc
...
def find_s_optimum_from_phases_via_counts(dt: float, edu_sp: float, dp: float, brdu_sp: float, dn_active: float, print_only=False):
    '''
    Function takes the following:
    ...
    dp - the number of double-positive cells counted, processed as above
    brdu_sp - the number of BrdU single-positive cells counted, processed as above
    dn_active - the nubmer of unlabeled cells beleived to be in G1/G2/M phase counted.
    ...
    # # From Harris et al., 2018. Appropriate only if cell cycle progression is reasonably clock-like with negligible growth in population.
    # ts_estimate = dt * (dp)/edu_sp # Only valid if edu_sp divisions are minimal or corrected for in the provided count
    # tc_estimate = ts_estimate * (edu_sp + dn_active)/(dp + brdu_sp)
    ...
    lookup_slice = reference_df[reference_df['BrdU time']==2].copy() # Picking a specific waiting time to narrow down number of rows to initially check
    # Scaled Euclidean distance emphasizes relative proportions rather than absolute counts, helping to generalize the tool
    lookup_slice['Relative ts difference'] = ((lookup_slice['ts'] - 24.0*ts_estimate/tc_estimate)/lookup_slice['ts'])**2
    nearest_row = lookup_slice.iloc[lookup_slice['Relative ts difference'].argmin(skipna=True)]
    comparable_series = reference_df[np.logical_and(np.isclose(reference_df['k1'], nearest_row['k1']), np.isclose(reference_df['k2'], nearest_row['k2']
                                                                                                                ))]
    max_snr = comparable_series['SNR k2'].max()
    dt_opt = comparable_series[comparable_series['SNR k2'] == max_snr]['BrdU time'].iloc[0]
    ...
    if print_only:
        readout(dt_opt, max_snr, counts=False)
        print('Phase times were deduced from counts using an exponentially growing population cell cycle model.')
    else:
        return dt_opt, max_snr
...
def find_custom_optimum_from_phases(tg1: float, ts: float, tc: float, g1_weight: float, s_weight: float, print_only=False):
    '''
    Function takes the following:
    tg1 - estimated duration of G1 phase
    ts - estimated duration of S phase
...
def readout(dt_opt, max_snr, dt=0, counts=True):

    print(f'A time of {dt_opt} hours is recommended, for which we found a SNR of {round(max_snr, 1)} for similar {'labeled cell' if counts else 'phase duration'} proportions in our simulation study.')
    if dt == dt_opt:
...
if __name__ == '__main__':
    ...
    # print('8.')
    # find_s_optimum_from_phases(tg1=4, ts=4.5, tc=8.5, print_only=True) # Mean numbers for all cells in bone marrow (Frindel, Generation cycle of mouse bone marrow, 1967)
    # # The tool recommends 2 hours, which conveniently is the most commonly used timing for studies on mouse cells.
    # print('9.')
    # find_custom_optimum_from_phases(tg1=4, ts=4.5, tc=8.5, g1_weight=1.0, s_weight=0.0, print_only=True) # Same numbers, now optimize k_g1
    # print('10.')
    # find_custom_optimum_from_phases(tg1=4, ts=4.5, tc=8.5, g1_weight=0.7, s_weight=0.3, print_only=True) # Same numbers, now optimize 70% k_g1 & 30% k_s

    print('11.')
    find_s_optimum_from_phases_via_counts(dt=2, edu_sp=6.30, dp=23.0, brdu_sp=6.59, dn_active=63.3, print_only=True)